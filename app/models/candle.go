...
func NewCandle(productCode string, duration time.Duration, datetime time.Time, open float64, close float64, high float64, low float64, volume float64) *Candle {
	return &Candle{
...
func (c *Candle) Update() error {
	cmd := fmt.Sprintf(`
		UPDATE %s SET open=?, close=?, high=?, low=?, volume=?
		WHERE time=?`, c.TableName())
